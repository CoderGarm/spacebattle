...
public class TransportationCache {
    ...
    @Nonnull
    private final CacheStore<Integer, Set<TransportJob>> cache = new CacheStore<>(2, TimeUnit.DAYS);
    ...
    @Nonnull
    public Set<TransportJob> get(@Nonnull final Tick today,
                                 final int idUser) {
        Preconditions.checkNotNull(today, "today must not be empty");

        return Objects.requireNonNullElse(cache.get(idUser), new HashSet<TransportJob>()).stream().filter(t -> t.isToday(today)).collect(Collectors.toSet());
    }
    ...
    private TransportJob getTodayTransportJob(@Nonnull final Tick today,
        ...
        Preconditions.checkState(from.getOwner().equals(to.getOwner()), " must not be empty");

        Set<TransportJob> transportJobs = cache.get(from.getOwner().getId());
        if (transportJobs == null) {
            transportJobs = new HashSet<>();
            cache.add(from.getOwner().getId(), transportJobs);
        }
