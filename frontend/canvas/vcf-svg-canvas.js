...
import {Element, List, Shape, Svg, SVG} from '@svgdotjs/svg.js';
import {event, select, zoom, zoomIdentity} from 'd3';
import './lib/svg.draggable';
import './lib/svg.clickable';
 ...
 */
class VcfSvgCanvas extends ElementMixin(ThemableMixin(PolymerElement)) {
    static get template() {
        return html`
      <style>
        :host {
          ...
          position: relative;
          overflow: hidden;
          --vcf-svg-canvas-toolbar-display: flex;
          --vcf-svg-canvas-toolbar-top: unset;
          --vcf-svg-canvas-toolbar-right: unset;
          --vcf-svg-canvas-toolbar-bottom: 0;
          --vcf-svg-canvas-toolbar-left: unset;
          --vcf-svg-canvas-toolbar-border-radius: 0 var(--lumo-border-radius) 0 0;
        }
        ...
        #toolbar {
          ...
          user-select: none;
          transition: all 0.2s;
          display: var(--vcf-svg-canvas-toolbar-display);
          top: var(--vcf-svg-canvas-toolbar-top);
          right: var(--vcf-svg-canvas-toolbar-right);
          bottom: var(--vcf-svg-canvas-toolbar-bottom);
          left: var(--vcf-svg-canvas-toolbar-left);
      ...
      <div id="toolbar" part="toolbar">
        <vaadin-button id="resetZoom" theme="tertiary icon" title="Reset Zoom" part="reset-zoom-button">
          <iron-icon icon="vcf-svg-canvas:bullseye"></iron-icon>
        </vaadin-button>
    ...
    /**
     * Toggle clickable events for `element`.
     * If `scale = true` will also zoom to fit element in viewport.
     *
     * @param {Element} element Element (SVG.js wrapper).
     * @param {Boolean} clickable Toggle clickable events.
     */
    clickable(element, clickable) {
        const clickEvents = ['clicked'];
        if (clickable && !element.hasClass('clickable')) {
            element.clickable(true);
            element.addClass('clickable');
            this._addElementEvents(element, clickEvents, false);
        } else if (!clickable) {
            element.clickable(false);
            element.removeClass('clickable');
            this._removeElementEvents(element, clickEvents, false);
        }
    }
    ...
    _addElementEvents(element, events, prefix = true) {
        events.forEach(eventName => {
            ...
            element.listeners[eventName] = e => {
                ...
                if (eventName === 'clicked' && Object.keys(element.listeners).includes('clicked')) {
                    element.clicked = true;
                } // todo no data in XHR event
    ...
    _setEventListeners(element) {
        ...
        this.clickable(element, element.attr('clickable') === 'true');
...
customElements.define(VcfSvgCanvas.is, VcfSvgCanvas);

/**
 * @namespace Vaadin
 */
window.Vaadin.VcfSvgCanvas = VcfSvgCanvas;
